...
# Enumerate the primary user and make it a local administrator for user profile migration 
$currentUser = (Get-WMIObject -Class Win32_ComputerSystem | Select-Object -ExpandProperty UserName)

## Check if the user is already in the Administrators group
$group = [ADSI]"WinNT://./Administrators,group"
$memberExists = $false

foreach ($member in $group.Members()) {
    if ($member.Name -eq $currentUser.Split('\')[1]) {
        $memberExists = $true
        break
    }
}

## If the user is not already in the Administrators group, add them
if (-not $memberExists) {
    net localgroup Administrators $currentUser /add 
    Write-Output "User $currentUser added to Administrators group."
} else {
    Write-Output "User $currentUser is already a member of the Administrators group."
}
